...
# Read files
directory = '/afs/hep.wisc.edu/home/incik/CMSSW_13_1_0_pre2/src/genTrainML/output/ggHtobbphi_dataset.h5'
f = h5py.File(directory, 'r')
#print(list(f.keys()))

x_grid = f['PhiRingEt'][:200000]
x_grid = x_grid.reshape(-1, 18, 1) #(1, 18, 1) # len() = 7573
...
layer1 = tf.keras.layers.Conv1D(filters = 16, kernel_size = 5, activation = "relu", padding = "valid")(x_gridinput)
# globalmax = tf.keras.layers.GlobalMaxPooling2D()(layer1)
flatten = tf.keras.layers.Flatten()(layer1)
x1input = tf.keras.Input(shape = x1_train.shape[1:])
concat = tf.keras.layers.Concatenate()([x1input, flatten])
layer2 = tf.keras.layers.Dense(32, activation = "relu")(concat)
output = tf.keras.layers.Dense(1)(layer2)

model = tf.keras.Model(inputs=(x_gridinput, x1input), outputs=output, name="multi-conv1d-f16-k3-d32")
...
plt.savefig("lCmulti-conv1d-f16-k3-d32.png")
...
tf.keras.utils.plot_model(model, to_file="multi-conv1d-f16-k3-d32.png", show_shapes=True)
model.save("multi-conv1d-f16-k3-d32_NN")
print("Done!")