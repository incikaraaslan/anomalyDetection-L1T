...
ptPad = ROOT.TPad("Pad 1", "Pad 1", 0, 0, 0.5, 1.0)
ptPad.SetLogy()
histpt = ROOT.TH1F("PUPPI P_T", "P_T (GeV)", 100, 0.0, 170.0)
ptPad.Draw()

canvas.cd()
phiPad = ROOT.TPad("Pad 2", "Pad 2", 0.5, 0, 1.0, 1.0)
phiPad.SetLogy()
histphi = ROOT.TH1F("PUPPI N_jets", "N_jets", 100, -4.0, 4.0)
phiPad.Draw()

canvas.cd()
"""etaPad = ROOT.TPad("Pad 3", "Pad 3", 0, 0, 1, 0.33)
etaPad.SetLogy()
histeta = ROOT.TH1F("PUPPI JetM", "Jet Multiplicity", 100, -5.0, 5.0)
etaPad.Draw()
canvas.cd()"""
...
for i in tqdm(range(chains['puppiJet'].GetEntries())): # used to be 'caloJet'
    ...
    canvas.cd()
    phiPad.cd()
    histphi.Fill(chains['puppiJet'].nObjects)

    """for m in range(chains['puppiJet'].nObjects.size()):
        canvas.cd()
        etaPad.cd()
...
for i in range(3):
    if i == 0:
        ...
        hists[i].SetTitle("")
        hists[i].GetYaxis().SetTitle("Events")
        hists[i].GetXaxis().SetTitle("N_jets")
        hists[i].GetXaxis().SetTitleSize(0.045)
        hists[i].GetXaxis().SetTitleOffset(0.9)
        hists[i].GetYaxis().SetTitleSize(0.045)
        hists[i].GetYaxis().SetTitleOffset(0.9)
...
canvas.Draw()
canvas.SaveAs('../genTrainML/controlPlots/PUPPIJet_ControlPlotjet.png')
print("PUPPI Done")

hists2 = []

# Creating Canvas Pads
canvas2 = ROOT.TCanvas('Ca', 'Canvas', 500, 500)

ptPad2 = ROOT.TPad("TPad 1", "Pad 1", 0, 0, 0.5, 1.0)
ptPad2.SetLogy()
histpt2 = ROOT.TH1F("TRIG P_T", "P_T (GeV)", 100, 0.0, 170.0)
ptPad2.Draw()

canvas2.cd()
phiPad2 = ROOT.TPad("TPad 2", "Pad 2", 0.5, 0, 1.0, 1.0)
phiPad2.SetLogy()
histphi2 = ROOT.TH1F("TRIG N_jets", "N_jets", 100, -4.0, 4.0)
phiPad2.Draw()
...
for i in tqdm(range(chains['trigJet'].GetEntries())): # used to be 'caloJet'
    ...
    canvas2.cd()
    phiPad2.cd()
    type(chains['trigJet'].nJets)
    type(chains['trigJet'].nJets[0])
    #histphi2.Fill()
...
for i in range(3):
    if i == 0:
        ...
        hists2[i].SetTitle("")
        hists2[i].GetYaxis().SetTitle("Events")
        hists2[i].GetXaxis().SetTitle("N_jets")
        hists2[i].GetXaxis().SetTitleSize(0.045)
        hists2[i].GetXaxis().SetTitleOffset(0.9)
        hists2[i].GetYaxis().SetTitleSize(0.045)
        hists2[i].GetYaxis().SetTitleOffset(0.9)
...
canvas2.Draw()
canvas2.SaveAs('../genTrainML/controlPlots/TRIGJet_ControlPlotjet.png')
print("Trig Done.")